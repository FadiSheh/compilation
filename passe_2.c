	...
	if(!root){
		...
		case NODE_PLUS:
			...
			if(root->opr[0]->nature == NODE_IDENT && root->opr[1]->nature == NODE_INTVAL){
				...
				create_inst_addu(r1, r1, r2);
				create_inst_sw(r1, tmp2->offset, gloc);
				liberer_reg(flagR1);
				liberer_reg(flagR2);
			}
			else{
				...
				r1 = return_reg1(r1);
				r2 = return_reg2(r2);
				create_inst_lw(r1, tmp2->offset, gloc);
